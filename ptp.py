...
class AttentionStore(AttentionControl):
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str):
        ...
        if is_cross and place_in_unet in ('up', 'down'):
            # self.step_store[key].append(attn)
            # import pdb; pdb.set_trace()
            self.step_store[key].append(attn.cpu()[:,:,:10])
        return attn
...
def aggregate_attention(attention_store: AttentionStore, res: int, from_where: List[str], is_cross: bool, select: int):
    ...
    # for item in attention_store.attention_store:
    #     for key, value in item.items():
    #         item[key] = [x.numpy() for x in value]
    # import pdb; pdb.set_trace()
    # import pickle
    # with open('attention_map.pkl', 'wb') as f:
    #     pickle.dump(attention_store.attention_store, f)
...
def show_self_attention_comp(attention_store: AttentionStore, res: int, from_where: List[str],
    ...
    for i in range(max_com):
        ...
        images.append(image)
    ptp_utils.view_images(np.concatenate(images, axis=1), image_name='self_attention.png')
...
def run_and_display(prompts, controller, latent=None, run_baseline=False, generator=None):
    ...
    ptp_utils.view_images(images, image_name='image.png')
    return images, x_t
...
def show_cross_attention(attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0):
    ...
    for i in range(len(tokens)):
        ...
        images.append(image)
    ptp_utils.view_images(np.stack(images, axis=0), image_name='cross_attention.png')
...
pass
controller.attention_store['down_cross'][0].shape
