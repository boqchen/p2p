...
ldm_stable = AutoPipelineForText2Image.from_pretrained("runwayml/stable-diffusion-v1-5", use_auth_token=MY_TOKEN, scheduler=scheduler).to(device)
ldm_stable.load_lora_weights('bochen123/stable-diffusion-v1-5-path-lora')
...
def show_cross_attention(attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0, image_name='image_attn.png'):
...
def run_and_display(prompts, controller, latent=None, run_baseline=False, generator=None, uncond_embeddings=None, verbose=True, image_name='image.png'):
    if run_baseline:
...
image_path = "HE.png"
prompt = "A H&E stained histopathology image."
...
ptp_utils.view_images([image_gt, image_enc, image_inv[0]], image_name='image_recon.png')
show_cross_attention(controller, 16, ["up", "down"], image_name='image_attn.png')
...
# prompts = ["a sagittal slice of lumbar spine CT scan",
#            "a sagittal slice of lumbar spine T2 weighted CT scan"
#         ]

prompts = ["A H&E stained histopathology image.",
           "A IMC stained histopathology image.",
        ]

cross_replace_steps = {'default_': .5,}
self_replace_steps = .4
blend_word = ((('H&E',), ("IMC",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
eq_params = {"words": ("IMC",), "values": (2.5,)} # amplify attention to the word "tiger" by *2 

with torch.cuda.amp.autocast():
    controller = make_controller(prompts, True, cross_replace_steps, self_replace_steps, blend_word, eq_params)
    images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings, image_name='modify_image.png')
